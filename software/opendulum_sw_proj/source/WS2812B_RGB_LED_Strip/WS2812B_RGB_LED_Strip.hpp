...
{
    public:
        WS2812B_RGB_LED_Strip(GPIO_Pin * gpio_pin_class_ptr_param);
    ...
    private:
        /** Return false if the gpio pin class didn't initialize correctly. */
        bool verify_gpio_pin_configuration_and_get_dma_dest_ptr(void);
        ...
        GPIO_Pin *gpio_pin_class_ptr;
        TIM_HandleTypeDef timer_handle; // used for triggering dma every ~300ns
        DMA_HandleTypeDef dma_handle;

        /** This variable tracks if we are expecting 
         * the DMA to throw a "Transfer Complete Flag,"
         * since we enabled the DMA channel. */
        bool has_dma_transfer_been_started; 
        
        uint32_t gpio_dma_bsrr_address_as_u32;
